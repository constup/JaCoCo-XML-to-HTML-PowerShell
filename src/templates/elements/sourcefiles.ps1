...
function Render-SourcefileRow {
    ...
    $name = $object.'@name'
    $pageLocation = "sources/$name.html"
    $pageLocation = Render-AssetPath -template $pageLocation -asset $pageLocation -directoryDepth $directoryDepth
...
function Render-SourcefilesCard {
    param (
        [Parameter(Mandatory=$true)]
        [String]$cardTemplate,
        [Parameter(Mandatory=$true)]
    ...
    foreach ($object in $objects) {
        $statisticsRow = Render-SourcefileRow `
